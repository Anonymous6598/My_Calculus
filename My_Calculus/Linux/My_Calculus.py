...
import customtkinter, tkinter, pickle, unicodedata, sys, numpy, matplotlib, matplotlib.pyplot, typing, My_Calculus_interface, re, My_Calculus_AI_window, warnings
...
class Program(customtkinter.CTk, My_Calculus_interface.My_Calculus_interface):
    ...
    def __button_operation__(self: typing.Self, button: str) -> None:
        ...
        match self.main_screen_expression_entry_data:
            case "%":
                ...
                if language_data == f"Српски": self.main_screen_result_entry.insert(f"0", f"результат {self.main_screen_result}")

                elif language_data == f"Русский": self.main_screen_result_entry.insert(f"0", f"результат {self.main_screen_result}")
    ...
    def __equation__(self: typing.Self) -> None:
        ...
        self.main_screen_result_entry.configure(state=f"normal")
        self.main_screen_result_entry.delete(f"0", tkinter.END)
        if language_data == f"Српски": self.main_screen_result_entry.insert(f"0", f"резултат је {self.main_screen_result}")
    ...
    def __change_base__(self: typing.Self, configure: str) -> None:
        ...
        self.main_screen_result_entry.configure(state=f"normal")
        self.main_screen_result_entry.delete(f"0", tkinter.END)
        if language_data == f"Српски": self.main_screen_result_entry.insert("0", f"резултат је {self.main_screen_count_system_result}")

        elif language_data == f"Русский": self.main_screen_result_entry.insert("0", f"результат {self.main_screen_count_system_result}")
...
class Graphical_claculator_adittional_layout(customtkinter.CTkToplevel):
    ...
    def __init__(self: typing.Self, *args, **kwargs) -> None:
        ...
        if language_data == f"Српски": self.main_screen_functions_text.configure(text=f"Функције")

        elif language_data == f"Русский": self.main_screen_functions_text.configure(text=f"Функции")
